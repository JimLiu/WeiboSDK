...
- (id)initWithCoder:(NSCoder *)decoder {
	if (self = [super init]) {
		...
		truncated = [decoder decodeBoolForKey:@"truncated"];
		latitude = [decoder decodeDoubleForKey:@"latitude"];
		longitude = [decoder decodeDoubleForKey:@"longitude"];
		inReplyToStatusId = [decoder decodeInt64ForKey:@"inReplyToStatusId"];
		inReplyToUserId = [decoder decodeIntForKey:@"inReplyToUserId"];
		inReplyToScreenName = [[decoder decodeObjectForKey:@"inReplyToScreenName"] retain];
		thumbnailPic = [[decoder decodeObjectForKey:@"thumbnailPic"] retain];
		bmiddlePic = [[decoder decodeObjectForKey:@"bmiddlePic"] retain];
		originalPic = [[decoder decodeObjectForKey:@"originalPic"] retain];
		user = [[decoder decodeObjectForKey:@"user"] retain];
		retweetedStatus = [[decoder decodeObjectForKey:@"retweetedStatus"]retain];
		commentsCount = [decoder decodeIntForKey:@"commentsCount"];
		retweetsCount = [decoder decodeIntForKey:@"retweetsCount"];
		unread = [decoder decodeBoolForKey:@"unread"];
		hasReply = [decoder decodeBoolForKey:@"hasReply"];
	}
	return self;
}
...
+ (Status*)statusWithStatusId:(long long)statusId {
    ...
    Status *status = [StatusCache get:[NSNumber numberWithLongLong:statusId]];
