...
- (void)viewDidLoad
{
    ...
    NSString *html = [NSString stringWithFormat:@"<html><body><div align=center>%@</div></body></html>", NSLocalizedString(@"SignInWebView_InitialMessageHTMLString", @"InitialMessageHTMLString")];
...
{
	self.title = NSLocalizedString(@"WebView_Loading", @"Loading");
    ...
    if (!HUD) {
        ...
        HUD.delegate = self;
        HUD.labelText = NSLocalizedString(@"WebView_Loading", @"Loading");
        [HUD show:YES];
    }
}
...
- (void)webView:(UIWebView *)aWebView didFailLoadWithError:(NSError *)error
{
    if (error.code != NSURLErrorCancelled && !_closed) {
        self.title = NSLocalizedString(@"WebView_FailedToLoad", @"Failed to load web page");
        self.navigationItem.rightBarButtonItem = _refreshButton;
        HUD.labelText = NSLocalizedString(@"WebView_FailedToLoad", @"Failed to load web page");
